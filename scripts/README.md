...
| file | what it is |
|---|---|
| `kiln-install.sh` | the one-command installer. **Phase 1** (stock kernel, online) pre-downloads an offline cache into `/opt/kiln`, installs the Kiln mainline 7.1.3 kernel, then hands off. **Phase 2** (patched kernel) builds the `rknpu` DKMS driver + runtimes + demos and restores wifi, **fully offline** from that cache. |
...
| `kiln-doctor` | ✓/✗ health check; exits non-zero on any critical fault. See [`../docs/TOOLS.md`](../docs/TOOLS.md). |

...
| 文档 | 内容 |
|---|---|
| [安装](INSTALL.md) | 在 Armbian 上一条命令安装(两次自动重启)、刷卡镜像、手动分阶段、各种 `KILN_*` 开关。 |
...
| 文档 | 内容 |
|---|---|
| [工具](TOOLS.md) | `kiln`(总启动器)、`kiln-doctor`(健康检查)、`kiln-config`(配置 TUI)、`kiln-convert`(板上转模型)。 |
| [配置](CONFIG.md) | `/etc/kiln/config.ini` —— 每个字段及编辑方式。 |
| [对话](CHAT.md) | `kiln-chat` —— 交互式大模型 CLI 和斜杠命令。 |
| [服务](SERVER.md) | `kiln-serve` —— OpenAI 兼容 HTTP API。 |
| [视觉](VISION.md) | `kiln-vision` —— 图像分类 + YOLO 目标检测,以及如何转 `.rknn`。 |
| [性能](BENCHMARK.md) | 实测 tok/s、ms/fps 及复现方法。 |

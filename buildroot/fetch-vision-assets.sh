...
# pre-converted rk3576 .rknn, so you convert it once on an x86 host with
# rknn-toolkit2 (see the note printed at the end) and drop the result in model/.
set -euo pipefail
...
[kiln] You still need a MobileNet .rknn for RK3576. Convert once on an x86 host
       with rknn-toolkit2 (get mobilenetv2-12.onnx via
       examples/mobilenet/model/download_model.sh), roughly:
